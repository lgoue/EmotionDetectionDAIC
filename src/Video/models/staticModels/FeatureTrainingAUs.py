"""
 ...
 python3 src/Video/models/staticModels/FeatureTrainingAUs.py --AUs_dir data/Ravdess/processed_AUs --model_number 11  --param (80) --type_of_norm 1 --out_dir /data/models/avg_MLP80_AUs/posteriors
"""
...
def process_AUs_avg(embs_path, avg_embs_path):
    """
        ...
        """
    if False: #(os.path.exists(avg_embs_path)):
    ...
    X_total["index"] = 0
    X_total["path"] = ""
    print(X_total["video_name"])
    X_total["actor"] = X_total["video_name"].apply(lambda x : pd.to_numeric(x.split("_")[0]))
    X_total["emotion"] = -1
    return X_total
...
if __name__ == '__main__':
    ...
    X_train, y_train, _ = clean_df(X_total)
